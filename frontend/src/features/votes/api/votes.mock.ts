...
export const voteHandlers = [
  ...
  http.get("/api/votes/:id", ({ params }) => {
    const { id } = params;
    const vote = mockVotes.find((v) => v.id === Number(id));
  ...
  http.post("/api/votes", async ({ request }) => {
    ...
    const newVoteId = mockVotes.length + 1;
    const newVote: Vote = {
      id: newVoteId,
      title: body.title,
      status: "OPEN",
    ...
    return HttpResponse.json({ id: newVoteId }, { status: 201 });
  }),

  // 투표 종료
  http.patch("/api/votes/:id/close", async ({ params, request }) => {
    const { id } = params;
    const body = (await request.json()) as { authorId: string };
    const voteIndex = mockVotes.findIndex((vote) => vote.id === Number(id));

    if (voteIndex === -1) {
      return new HttpResponse(JSON.stringify("존재하지 않는 투표입니다."), { status: 404 });
    }

    // authorId 검증 (실제로는 백엔드에서 처리)
    if (mockVotes[voteIndex].authorId !== body.authorId) {
      return new HttpResponse(JSON.stringify("투표를 종료할 권한이 없습니다."), { status: 403 });
    }
    ...
    return HttpResponse.json({ id: closedVote.id, status: closedVote.status });
  }),
  ...
  http.post("/api/votes/:id/vote", async ({ params, request }) => {
    ...
    const existingRecordIndex = mockVoteRecords.findIndex(
      (record) =>
        record.voteId === voteId && record.voterId === body.voterId
    );
    ...
    const record = mockVoteRecords.find(
      (r) => r.voteId === voteId && r.voterId === body.voterId
    );
  ...
  http.get("/api/votes/:id/vote-record", ({ params, request }) => {
    ...
    const record = mockVoteRecords.find(
      (r) => r.voteId === voteId && r.voterId === voterId
    );
